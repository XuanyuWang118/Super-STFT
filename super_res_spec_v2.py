...
class SuperResDecoder(nn.Module):
    ...
    def _init_weights(self):
        ...
        k = np.arange(self.n_fft // 2 + 1)[:, None]
        window = np.hanning(self.n_fft)
        norm_factor = 1.0 / self.n_fft 
        real_part = np.cos(2 * np.pi * k * n / self.n_fft) * window * norm_factor
        imag_part = np.sin(2 * np.pi * k * n / self.n_fft) * window * norm_factor
        weight = np.concatenate([real_part, imag_part], axis=0)
        self.dec.weight.data = torch.from_numpy(weight).float().unsqueeze(1)
        self.dec.weight.requires_grad = True
...
class MultiResConsistencyLoss(nn.Module):
    ...
    def forward(self, pred_super_complex, raw_audio, verbose=False):
        ...
        for i, (win_ms, hop_ms) in enumerate(self.config.target_resolutions):
            ...
            gt_view = torch.view_as_real(gt_complex_tensor) # (B, F, T, 2) 
            ...
            if time_factor > 1:
                ...
                curr_pred_complex = curr_pred_complex[:, :, :n_blocks*time_factor]
